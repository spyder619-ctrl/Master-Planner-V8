...
function loadState(){
  try{
    ...
    // Sanity: protect against corrupted localStorage (null/invalid objects)
    if(!merged.checks || typeof merged.checks !== "object") merged.checks = {};
    if(!merged.groceryPrices || typeof merged.groceryPrices !== "object") merged.groceryPrices = base.groceryPrices;
    if(!merged.supplementPrices || typeof merged.supplementPrices !== "object") merged.supplementPrices = base.supplementPrices;
    if(!merged.storageChecklists || typeof merged.storageChecklists !== "object") merged.storageChecklists = base.storageChecklists;
    if(!merged.storageQty || typeof merged.storageQty !== "object") merged.storageQty = { grocery:{}, supplements:{} };
    if(!merged.storageQty.grocery || typeof merged.storageQty.grocery !== "object") merged.storageQty.grocery = {};
    if(!merged.storageQty.supplements || typeof merged.storageQty.supplements !== "object") merged.storageQty.supplements = {};
    return merged;
  }catch(e){
...
function renderError(e){
  try{
    console.error(e);
    setHeader("Error", "Something broke while loading this page.");
    const msg = (e && (e.stack || e.message)) ? (e.stack || e.message) : String(e);
    $("#app").innerHTML = card(`
      <div class="h1">Page failed to load</div>
      <p class="sub">This is usually a cached build. Try refreshing once, or reopen the app.</p>
      <pre class="codeBlock">${escapeHtml(msg)}</pre>
      <div class="toolbar">
        <a class="btn" href="#home">Home</a>
        <a class="btn" href="#schedule">Schedule</a>
        <a class="btn" href="#grocery">Grocery</a>
...
function schedClass(title, meta){
  ...
  if(has("sleep") || has("bed") || has("night")) return "entry-sleep";
  if(has("workout") || has("gym") || has("run") || has("cardio") || has("lift")) return "entry-workout";
  if(has("shake") || has("breakfast") || has("lunch") || has("dinner") || has("meal") || has("snack")) return "entry-meal";
  ...
  if(has("shower") || has("groom") || has("brush") || has("skincare")) return "entry-hygiene";
  if(has("work")) return "entry-work";
  if(has("personal") || has("free time") || has("relax")) return "entry-personal";
  return "entry-other";
}
...
async function render(page, arg){
  ...
  switch(page){
    ...
    case "appendix": return await renderAppendix();
    case "schedule": return await renderSchedule();
    case "day": return await renderDay(decodeURIComponent(arg||""));
    case "grocery": return await renderGrocery();
    case "groceryStorage": return await renderGroceryStorage();
    case "supplements": return await renderSupplements();
    case "suppStorage": return await renderSuppStorage();
    case "nutrition": return await renderNutrition();
    case "recipe": return await renderRecipe(decodeURIComponent(arg||""));
    case "workouts": return await renderWorkouts();
    case "workout": return await renderWorkout(decodeURIComponent(arg||""));
